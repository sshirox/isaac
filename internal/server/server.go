...
func Run() error {
	...
	s, err := storage.NewMemStorage(gaugeStore, counterStore, gauges, counters)
	...
	r.Post("/update", logger.WithLogging(handler.UpdateMetricsJSONHandler(uc)))
	r.Post("/value", logger.WithLogging(handler.GetMetricsJSONHandler(uc)))
